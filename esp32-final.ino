...
const char* server_address = "https://noise-wearable.herokuapp.com/api/noise_observation/";
...
#define MIC_TIMING_SHIFT  0           // Set to onrate to fix MSB timing for some microphones, i.e. SPH0645LM4H-x
...
// I2S peripheral to use (0 or 1)E
...
void setup() {
  ...
  // set correct baud for reading from terminal
  Serial.begin(112500);
  delay(1000); // Safety
  ...
  // Begin creating json object to send data to server
  DynamicJsonDocument doc(2004);
  doc["user_id"] = deviceName;
  ...
  while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
      ...
      JsonObject obs = values.createNestedObject();

      timeClient.update();
      obs["time_obs"] = timeClient.getEpochTime();
      obs["db_reading"] = Leq_dB;
      count++;

      // only send off every 32 recordings
      if (count == 32) {
        ...
        doc = DynamicJsonDocument(2004);
        doc["user_id"] = deviceName;
        values = doc.createNestedArray("data");
        count = 0;
      }
